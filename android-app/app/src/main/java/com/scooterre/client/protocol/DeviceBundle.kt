...
object DeviceBundle {
    ...
    fun export(context: Context, mac: String, out: OutputStream, password: String?): Boolean {
        ...
        val (ref, totals) = BatteryHistoryStore(context).exportRaw(device.mac)
        ...
        ref?.let { manifest.put("efficiencyRef", JSONObject(it)) }
    ...
    fun import(context: Context, data: ByteArray, password: String?): ImportResult {
        ...
        return try {
            ...
            BatteryHistoryStore(context).importRaw(mac, manifest.optJSONObject("efficiencyRef")?.toString(), manifest.optJSONObject("efficiencyTotals")?.toString())
