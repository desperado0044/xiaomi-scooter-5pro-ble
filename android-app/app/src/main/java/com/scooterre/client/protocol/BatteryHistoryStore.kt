...
data class ModeEfficiencyTotals(val totalKm: Double, val totalWh: Double) {
...
class BatteryHistoryStore(context: Context) {
    ...
    fun attributeRide(mac: String, mode: Long, delta: PendingRideDelta, totalMileageKm: Double, remainingBatteryMah: Long, voltage: Double) {
        ...
        current[mode] = ModeEfficiencyTotals(existing.totalKm + delta.km, existing.totalWh + delta.wh)
    ...
    fun totals(mac: String): Map<Long, ModeEfficiencyTotals> {
        ...
        for (key in o.keys()) {
            ...
            result[key.toLong()] = ModeEfficiencyTotals(entry.getDouble("km"), entry.getDouble("wh"))
        }
        return result
    }
